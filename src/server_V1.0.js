...
// Middleware to check if tunnel is ready
/* app.use((req, res, next) => {
  if (!tunnelStream) {
    return res.status(503).send("SSH tunnel not ready");
  }
  next();
}); */
...
// Test endpoint that forwards to department server's /hello
app.get("/test", (req, res) => {
  const options = {
    ...
    path: "/hello",
    method: "GET",
    socket: tunnelStream, // Use the SSH tunnel stream
    agent: false,
  };

  const proxyReq = http.request(options, (proxyRes) => {
    console.log(
      `Forwarded /test to department server /hello - Status: ${proxyRes.statusCode}`
    );
    proxyRes.pipe(res);
  });
  ...
  proxyReq.on("error", (err) => {
    ...
    res.status(502).send("Failed to forward request to department server");
  });
